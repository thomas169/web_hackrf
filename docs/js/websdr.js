...
// ── Meters ───────────────────────────────────────────────────
const dom_tps_val = document.getElementById("tpsVal");
const dom_tps_fill = document.getElementById("tpsFill");
const dom_mbs_val = document.getElementById("mbsVal");
const DB_MIN = -90;
const DB_MAX = 10;
...
function saveSettings() {
    for (const el of Object.values(save_elements)) {
        if (el?.id)
            localStorage.setItem(storageKey(el.id), el.value);
    }
}
...
function ensureScopeGrid(w, h) {
    if (scopeGridCache?.width === w && scopeGridCache?.height === h)
        return;
...
function updateMeters(r) {
    if (r > peakRMS) {
        ...
        peakRMS = Math.max(peakRMS * 0.98, 0);
...
async function readLoop() {
    ...
    const transfer = async () => {
        while (rxRunning && sdr) {
            ...
            if (isRecording)
                recChunks.push(new Uint8Array(result));
            for (let i = 0; i < iq.length; i++)
                iqHistory.push(iq[i]);
            const maxSamples = (CHUNK_BYTES / BYTES_PER_SAMPLE) * IQ_TRANSFERS;
            if (iqHistory.length > maxSamples)
                iqHistory.splice(0, iqHistory.length - maxSamples);
            updateMeters(r);
            updateTPS();
        }
    };
    ...
    Array.from({ length: RX_JOB_SIZE }, transfer)
        .forEach(p => p.catch((e) => log("Transfer error: " + e, "err")));
    drawLoop().catch((e) => log("Draw error: " + e, "err"));
...
async function update_param(name) {
    saveSettings();
    if (!sdr)
        return;
